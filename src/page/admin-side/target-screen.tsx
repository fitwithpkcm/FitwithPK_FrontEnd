...
import {
  ...
  BarChart2, Footprints, Droplets, Moon, MoveHorizontal, Flame, RefreshCw, AlertTriangle
} from "lucide-react";
...
import { getDietPlan, getDailyUpdateForAWeek, getWeeklyUpdate } from "../../services/UpdateServices";
...
function CalorieMacroTargetCard({ bmr, latestMeasurement, value, onChange }: CalorieMacroTargetCardProps) {
...
export default function UserTargetsScreen() {
  ...
  // Latest weekly measurement — BMR is computed server-side (getWeeklyStatus)
  // and is the basis for the calorie target.
  const { data: weeklyMeasurements } = useQuery<IBodyMeasurement[]>({
    queryKey: ["target-weekly-measurements", selectedClient?.IdUser],
    queryFn: () => getWeeklyUpdate({ IdUser: selectedClient!.IdUser }).then(res => res.data.data),
    enabled: !!selectedClient?.IdUser,
  });
  const latestMeasurement = weeklyMeasurements?.[weeklyMeasurements.length - 1];
  ...
  // Validate the macro split and stamp the calorie target with the weekly
  // measurement it was based on (used later to flag a stale target).
  const prepareTargetsForSave = (): UserTarget | null => {
    if (!targets.nutrition) return targets;
    if (!isMacroSplitValid(targets.nutrition)) {
      alert("Protein + carbs + fat must total 100% before saving the calorie target.");
      return null;
    }
    return {
      ...targets,
      nutrition: {
        ...targets.nutrition,
        basisWeight: latestMeasurement?.Weight ?? targets.nutrition.basisWeight,
        basisDate: latestMeasurement?.DateRange ?? targets.nutrition.basisDate,
      },
    };
  };
  ...
  const handleSaveTargets = () => {
    ...
    const toSave = prepareTargetsForSave();
    if (!toSave) return;
    const fd = new FormData();
    fd.append("IdUser", selectedClient.IdUser!.toString());
    fd.append("Target", JSON.stringify(toSave));
        ...
        <div className="p-4 space-y-4">
          ...
          {activeTab === "targets" && (
            <div className="space-y-4">
              ...
              <CalorieMacroTargetCard
                bmr={latestMeasurement?.BMR ?? null}
                latestMeasurement={latestMeasurement}
                value={targets.nutrition}
                onChange={n => setTargets(p => ({ ...p, nutrition: n }))}
              />
