    ...
    <div className="fixed bottom-0 left-0 right-0 z-20">
      <nav className="bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 shadow-lg">
        <div className="flex justify-around px-1 py-1">
          ...
          <NavItem icon={<ArrowRightLeft className="h-5 w-5" />} label="NutriSwap" href={RENDER_URL.ADMIN_NUTRISWAP} isActive={location === RENDER_URL.ADMIN_NUTRISWAP} onNavAttempt={onNavAttempt} />
...
function NavItem({ icon, label, href, isActive, onNavAttempt }: NavItemProps) {
  ...
  return (
    <Link href={href} onClick={handleClick} className="flex flex-col items-center py-2 px-2 gap-0.5">
      <div
